...
import { Seller, FlaggedQuote, AdminMetrics, OrderDispute } from '../types';
import { api } from '../services/api';
import { useToast } from '../context/ToastContext';

export const AdminPortal: React.FC = () => {
  const { showToast } = useToast();
  const [activeSection, setActiveSection] = useState<'overview' | 'sellers' | 'moderation' | 'disputes'>('overview');
  ...
  const handleSellerAction = async (id: string, action: 'approve' | 'reject' | 'suspend' | 'reinstate') => {
    try {
      ...
      console.error(err);
      showToast('error', 'Failed to update seller');
    }
  };
  ...
  const handleModerate = async (id: string, decision: 'approved' | 'rejected') => {
    try {
      ...
      console.error(err);
      showToast('error', 'Failed to process moderation');
    }
  };
                    ...
                    <tr key={s.id}>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          {s.status === 'suspended' && (
                            <button onClick={() => handleSellerAction(s.id, 'reinstate')} className="bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded-lg font-bold text-[11px] cursor-pointer">Reinstate</button>
                          )}
                        </div>
                      </td>
                    </tr>
